...
class DataBase {
  ...
  return {
    ...
    'storedShopOptions': Map.from(storedShopOptions).map((key, value) => MapEntry(key, value.toJson())),
    'storedTasks': storedTasks.map((task) => task.toJson()).toList(),
  };
}
  ...
  Future<void> updateDatabase() async {
    ...
    //var response = await http.post(url, body: jsonEncode(toJson()));
    var response = await http.post(Uri.parse('$url$USER_ID'), body: toJson());
...
class DataBase {
  ...
  Future<void> updateCloud() async {
    var url = Uri.parse(DataBase.url + USER_ID.toString());
    print("sending put request");
    var response = await http.put(url, body: jsonEncode(toJson()));
